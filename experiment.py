...
def create_experiment(experiment_name, run_metrics, model, model_name, artifact_dir,
    ...
    with mlflow.start_run(run_name=name):
        ...
        if confusion_matrix_path is not None:
            mlflow.log_artifact(confusion_matrix_path.replace("\\", "/"), artifact_dir)
            
        if classification_report_plot_path is not None:
            mlflow.log_artifact(classification_report_plot_path.replace("\\", "/"), artifact_dir)
            
        if roc_auc_plot_path is not None:
            mlflow.log_artifact(roc_auc_plot_path.replace("\\", "/"), artifact_dir)
        
        if feature_importance_path is not None:
            mlflow.log_artifact(feature_importance_path.replace("\\", "/"), artifact_dir)
        
        mlflow.set_tag("tag1", experiment_name)
        
    print('Run - %s is logged to Experiment - %s' % (name, experiment_name))

df = pd.read_csv('df_nettoye.csv')
...
for model, model_name, param_grid in models:
    ...
    cm_path = os.path.join(artifact_dir, 'confusion_matrix.png').replace("\\", "/")
    cr_path = os.path.join(artifact_dir, 'classification_report.txt').replace("\\", "/")
    roc_auc_path = os.path.join(artifact_dir, 'roc_auc_plot.png').replace("\\", "/") if y_proba is not None else None
